...
function LogAnalysis(props) {
    ...
    useEffect(() => {
        updateDataSetInfo('scene', 'log_parsing');
        if (taskId) {
            getTbDetail(taskId, {
                cb: (data) => {
                    const {
                        taskName = '', description = undefined, scene = 'log_parsing',
    ...
    const renderParams = () => {
        let params = {
            ...
            dataSourceList: [{
                ...
                relationshipType: 'TIME_SERIES', //LOG_PATTERN
            }]
        };
        return params;
    };
